...
*Setup instructions for a gitian build of Merelcoin using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Merelcoin
...
- [Building Merelcoin](#building-merelcoin)
...
Clone the git repositories for merelcoin and gitian and then checkout the merelcoin version that you want to build.
...
Follow the instructions in [doc/release-process.md](release-process.md) in the merelcoin repository
...
To build Merelcoin (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the merelcoin repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/merelcoin/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/merelcoin-project/merelcoin
...
For example:
```bash
URL=https://github.com/user/merelcoin.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit merelcoin=${COMMIT} --url merelcoin=${URL} ../merelcoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit merelcoin=${COMMIT} --url merelcoin=${URL} ../merelcoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit merelcoin=${COMMIT} --url merelcoin=${URL} ../merelcoin/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/merelcoin-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/merelcoin-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/merelcoin-build.assert
```
...
[merelcoin/gitian.sigs](https://github.com/merelcoin-project/gitian.sigs.ltc/) repository, or if that's not possible create a pull
request. 
